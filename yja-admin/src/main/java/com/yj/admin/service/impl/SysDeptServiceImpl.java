...
import com.yj.core.converter.DeptConverter;
...
public class SysDeptServiceImpl extends ServiceImpl<SysDeptMapper, SysDept> implements SysDeptService {
     ...
     */
    @Override
    public List listDepartments(DeptQuery queryParams) {
        // 查询参数
        ...
        // 获取所有部门ID
        Set<String> deptIds = deptList.stream()
                .map(SysDept::getId)
                .collect(Collectors.toSet());
        // 获取父节点ID
        Set<String> parentIds = deptList.stream()
     ...
     * @return 部门树形列表
     */
    public List<DeptVO> recurDeptList(String parentId, List<SysDept> deptList) {
     ...
     * @return 部门表格层级列表
     */
    public static List<Option> recurDeptTreeOptions(String parentId, List<SysDept> deptList) {
        List<Option> list = CollectionUtil.emptyIfNull(deptList).stream()
                .filter(dept -> dept.getParentId().equals(parentId))
                .map(dept -> {
                    Option option = new Option(dept.getId(), dept.getName());
